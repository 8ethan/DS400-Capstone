...
class SectionInfo(FeatureType):
    ...
    def raw_features(self, bytez, lief_binary):
        ...
        try:
            if lief_binary.section_from_offset(lief_binary.entrypoint) is not None:
                entry_section = lief_binary.section_from_offset(lief_binary.entrypoint).name
            else:
                #pls help im gonna kms
                raise
        except:
...
class PEFeatureExtractor(object):
    ...
    def feature_vector_spark(self, bytez):
        return DenseVector(self.process_raw_features(self.raw_features(bytez)))
